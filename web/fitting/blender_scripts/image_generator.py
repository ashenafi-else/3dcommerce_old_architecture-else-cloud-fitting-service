...
if len(argv) == 0:
    ...
    print("")
    print("Example:")
    print("blender transparent_environment.blend -b -P coloring_by_distance.py -- './Mesh/LAST MAN LOW- SIZE 40.stl' './Mesh/model_r.stl'")
    print("")
    exit(-1)
...
for ver in model_mesh.verts:
    ...
    dist = dist if dist < 4 else 4

    model_colors[ver.index] = (1 - ((dist - 1) * (255 / 3)) / 255, 0, 0)
...
render_image(camera, (1920, 1080), './export')
...
bpy.ops.export_scene.b4w_html(filepath='./export', do_autosave=False)
